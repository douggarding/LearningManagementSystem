...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        private string getNextUserID(List<string> userIDs)
        {
            ...
            string lastID = userIDs[userIDs.Count];
            int lastIDValue = int.Parse(lastID.Substring(1, lastID.Length));
            if(lastIDValue + 1 == userIDs.Count) // +1 to account for u0000000
            {
                nextID = nextID + (lastIDValue + 1);
            }

            else // Go through list until a useable number is found
            {
                for (int i = 0; i < userIDs.Count; i++)
                {
                    string curr = userIDs[i];
                    int value = int.Parse(curr.Substring(1, curr.Length));
                    if (i < value) // Then we found a skipped ID number
                    {
                        nextID = nextID + i;
                    }
                }
